...
public sealed class DependencyIndexerTests
{
    public DependencyIndexerTests()
    {
        ...
        var dbSettings = Options.Create(new SaddleRagDbSettings());
        var contextFactory = new SaddleRagDbContextFactory(dbSettings);
    ...
    private static readonly string smTestCsprojPath =
        Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "SaddleRAG.Tests.csproj"));
